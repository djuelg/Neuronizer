...
public class DisplayHeadersInteractorImpl extends AbstractInteractor implements DisplayHeadersInteractor {
    ...
    private void postHeader(final List<TodoListHeader> headers) {
        mMainThread.post(new Runnable() {
            @Override
            public void run() {
                callback.onHeaderRetrieved(headers);
            }
        });
    }
    ...
    @Override
    public void run() {
        List<TodoListHeader> headers = repository.getHeadersOfTodoListId(todoListUuid);
        postHeader(headers);
    }
}
